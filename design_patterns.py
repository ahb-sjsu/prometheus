...
def _is_generated_file(filepath: str, content: str) -> bool:
    ...
    generated_markers = [
        ...
        "# DO NOT EDIT", "// DO NOT EDIT", "# Auto-generated",
        "// Auto-generated", "@generated", "This file is auto-generated",
        "Code generated by",
    ]
...
DESIGN_PATTERNS = {
    "python": {
        ...
        "observer_unsubscribe": re.compile(r"\.unsubscribe\s*\(|\.remove_observer\s*\(|\.unregister\s*\("),
    ...
    "java": {
        "singleton_instance": re.compile(r"private\s+static\s+\w+\s+instance|getInstance\s*\(\s*\)"),
        ...
        "god_class_methods": re.compile(r"^\s+(?:public|private|protected)\s+\w+\s+\w+\s*\(", re.MULTILINE),
        ...
        "factory_if_chain": re.compile(r'if\s*\([^)]+\.equals\s*\([\'"][^\'"]+[\'"]\)\s*\)\s*\{?\s*return\s+new'),
    ...
    "cpp": {
        "meyers_singleton": re.compile(r"static\s+\w+&?\s+getInstance|static\s+\w+&\s+instance\s*\(\s*\)"),
        "static_instance": re.compile(r"static\s+\w+\s*\*?\s*m?_?instance"),
        "god_class_methods": re.compile(r"^\s+(?:virtual\s+)?(?:\w+::)?\w+\s+\w+\s*\(", re.MULTILINE),
        "template_explosion": re.compile(r"template\s*<[^>]*<[^>]*<"),
        "sfinae_abuse": re.compile(r"std::enable_if|std::void_t|decltype\s*\([^)]*\.\.\.\)"),
        "multiple_inheritance": re.compile(r"class\s+\w+\s*:\s*(?:public|private|protected)\s+\w+\s*,"),
        ...
        "abstract_factory": re.compile(r"class\s+\w*(?:Abstract)?Factory|virtual\s+\w+\s*\*\s*create"),
...
class DesignPatternDetector:
    ...
    def detect(
        ...
        if not is_test:
            singleton_issues = self._check_singleton_abuse(content, filepath, language, lang_patterns, lines)
            issues.extend(singleton_issues)
            if singleton_issues:
                patterns_found.append("singleton")

        god_class_issues = self._check_god_class(content, filepath, language, lang_patterns, lines, is_test)
        issues.extend(god_class_issues)
        if god_class_issues:
            patterns_found.append("god_class")

        if not is_test:
            factory_issues = self._check_factory_abuse(content, filepath, language, lang_patterns, lines)
            issues.extend(factory_issues)
            if factory_issues:
                patterns_found.append("factory")

            inheritance_issues = self._check_inheritance_abuse(content, filepath, language, lang_patterns, lines)
            issues.extend(inheritance_issues)
            if inheritance_issues:
                patterns_found.append("inheritance")

            abstraction_issues = self._check_over_abstraction(content, filepath, language, lang_patterns, lines)
            issues.extend(abstraction_issues)
            if abstraction_issues:
                patterns_found.append("abstraction")

            observer_issues = self._check_observer_leaks(content, filepath, language, lang_patterns, lines)
            issues.extend(observer_issues)
            if observer_issues:
                patterns_found.append("observer")

            builder_issues = self._check_builder_bloat(content, filepath, language, lang_patterns, lines)
        ...
        if language == "python":
                ...
                for match in patterns["global_instance"].finditer(content):
                    ...
                    if not re.match(r'^[A-Z_]+\s*=\s*(?:True|False|None|\d+|[\'"])', matched_text):
                        issues.append(DesignPatternIssue(
                            pattern_type="singleton_abuse", severity="MEDIUM",
            ...
            if patterns.get("global_singleton"):
                for match in patterns["global_singleton"].finditer(content):
                    ...
                    line_num = self._find_line_number(content, match)
                    issues.append(DesignPatternIssue(
                        pattern_type="singleton_abuse", severity="HIGH",
                        file=filepath, line=line_num,
    ...
    def _check_god_class(
        self, content: str, filepath: str, language: str, patterns: dict, lines: list, is_test: bool = False
    ) -> list[DesignPatternIssue]:
        ...
        if not is_test and language == "python" and patterns.get("god_class_attrs"):
            ...
            if attr_count > self.THRESHOLDS["god_class_attrs"]:
                issues.append(DesignPatternIssue(
                    pattern_type="god_class", severity="MEDIUM",
                    file=filepath, line=1,
    ...
    def _check_factory_abuse(
        ...
        if factory_if_pattern:
            matches = [m for m in factory_if_pattern.finditer(content)
                       if should_flag_match(content, m, filepath)]
            if len(matches) >= 3:
                for match in matches[:1]:
                    line_num = self._find_line_number(content, match)
                    issues.append(DesignPatternIssue(
                        pattern_type="factory_overkill", severity="MEDIUM",
                        file=filepath, line=line_num,
        ...
        if "FactoryFactory" in content or "AbstractAbstract" in content:
            issues.append(DesignPatternIssue(
                pattern_type="factory_overkill", severity="HIGH",
                file=filepath, line=1,
        ...
        if language == "python" and patterns.get("mixin_explosion"):
            for match in patterns["mixin_explosion"].finditer(content):
                ...
                if parents > self.THRESHOLDS["mixin_count"]:
                    issues.append(DesignPatternIssue(
                        pattern_type="inheritance_abuse", severity="MEDIUM",
                        file=filepath, line=line_num,
    ...
    def _check_over_abstraction(
        ...
        if interface_pattern:
            matches = [m for m in interface_pattern.finditer(content)
                       if should_flag_match(content, m, filepath)]
            if len(matches) > self.THRESHOLDS["interface_count"]:
                issues.append(DesignPatternIssue(
                    pattern_type="over_abstraction", severity="MEDIUM",
                    file=filepath, line=1,
        ...
        if language == "python":
            ...
            if abstract_pattern:
                ...
                if abstract_count > 5:
                    issues.append(DesignPatternIssue(
                        pattern_type="over_abstraction", severity="LOW",
                        file=filepath, line=1,
    ...
    def _check_observer_leaks(
        ...
        unsubscribe_pattern = patterns.get("observer_unsubscribe") or patterns.get("remove_listener")
        ...
        if subscribe_pattern and unsubscribe_pattern:
            ...
            if subscribe_count > 0 and unsubscribe_count == 0:
                issues.append(DesignPatternIssue(
                    pattern_type="observer_leak", severity="HIGH",
                    file=filepath, line=1,
    ...
    def _check_builder_bloat(
        ...
        if builder_pattern and builder_pattern.search(content):
            ...
            if 1 <= setter_count <= 3:
                issues.append(DesignPatternIssue(
                    pattern_type="builder_bloat", severity="LOW",
                    file=filepath, line=1,
    ...
    def _check_cpp_specific(
        ...
        if patterns.get("raw_new"):
            raw_news = [m for m in patterns["raw_new"].finditer(content)
                        if should_flag_match(content, m, filepath)]
            smart_ptr_count = len(re.findall(r"unique_ptr|shared_ptr|make_unique|make_shared", content))

            if len(raw_news) > smart_ptr_count and len(raw_news) > 3:
                issues.append(DesignPatternIssue(
                    pattern_type="memory_management", severity="HIGH",
                    file=filepath,
        ...
        if patterns.get("template_explosion"):
            for match in patterns["template_explosion"].finditer(content):
                ...
                line_num = self._find_line_number(content, match)
                issues.append(DesignPatternIssue(
                    pattern_type="template_abuse", severity="MEDIUM",
                    file=filepath, line=line_num,
