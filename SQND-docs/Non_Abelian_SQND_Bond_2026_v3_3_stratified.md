...
#### 2.4.7 Cooling and Hysteresis

First-order phase transitions exhibit **hysteresis**. As moral temperature decreases post-crisis:
...
4. **Stratum-sequential ordering**: High-D boundaries (abstract principles) reconstitute first; low-D boundaries (concrete decisions) reconstitute last. This predicts that post-crisis societies recover moral clarity about principles before they recover clarity about applications.
...
5. **Critical scaling**: Near the local phase boundary:
$$\theta(T, d) \propto |T - T_c(d)|^{-\nu}$$
