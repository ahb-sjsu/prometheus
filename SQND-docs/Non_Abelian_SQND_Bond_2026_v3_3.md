...
*Added moral phase transitions via finite-temperature symmetry restoration*
...
### 2.4 Moral Phase Transitions

The boundary Higgs mechanism admits a remarkable extension: **temperature-dependent symmetry restoration**.

**Finite-Temperature Effective Potential**

At finite temperature $T$, the effective potential receives thermal corrections [8]:

$$V_{\text{eff}}(\phi, T) = \left(-\mu_\phi^2 + c T^2\right)|\phi|^2 + \lambda_\phi |\phi|^4$$

where $c > 0$ is a coefficient determined by the gauge and scalar couplings.

**Critical Temperature**

The coefficient of $|\phi|^2$ changes sign at the **critical temperature**:

$$T_c = \frac{\mu_\phi}{\sqrt{c}}$$

- For $T < T_c$: The potential has its minimum at $|\phi| = v(T) \neq 0$. Symmetry is **broken**. Boundaries are "rigid."

- For $T > T_c$: The minimum is at $|\phi| = 0$. Symmetry is **restored**. Boundaries become "transparent."

**Temperature-Dependent VEV**

Below the critical temperature:

$$v(T) = v_0 \sqrt{1 - \frac{T^2}{T_c^2}}$$

where $v_0 = \mu_\phi/\sqrt{2\lambda_\phi}$ is the zero-temperature VEV.

**Temperature-Dependent Masses**

The incidenton masses inherit this dependence:

$$m_I(T) = \frac{g_I v(T)}{2} = m_I(0) \sqrt{1 - \frac{T^2}{T_c^2}}$$

As $T \to T_c$: $m_I \to 0$. The boundary becomes **massless**—transparent to bond-type mixing.

---

**Definition 2.2 (Moral Temperature).** The *moral temperature* $T_{\text{moral}}$ of a social context is a measure of normative volatility, ambiguity, or crisis intensity. High moral temperature corresponds to:

- Rapid social change
- Institutional breakdown
- Competing normative frameworks
- High stakes with unclear precedents

---

**The Two Phases of Moral Boundaries**

| Phase | Temperature | VEV | Boundary Behavior |
|-------|-------------|-----|-------------------|
| **Ordered** (broken symmetry) | $T < T_c$ | $v \neq 0$ | Rigid; bond types preserved across threshold |
| **Disordered** (restored symmetry) | $T > T_c$ | $v = 0$ | Transparent; maximal bond-type mixing |

**Physical Predictions**

1. **Crisis-induced boundary dissolution**: In periods of extreme social upheaval (wars, revolutions, pandemics), moral boundaries that normally maintain clear distinctions (obligation vs. claim, duty vs. liberty) become porous. What was clearly "my duty" becomes ambiguously "your claim" or even "mere liberty."

2. **Hysteresis effects**: First-order phase transitions exhibit hysteresis. A society that has "melted" its moral boundaries may not immediately reconstitute them when the crisis passes. There can be **supercooling**—a period where $T < T_c$ but the system remains in the disordered phase.

3. **Domain walls**: If different regions of a society have different moral temperatures, **domain walls** can form between ordered and disordered phases. These are zones of intense moral contestation where the VEV interpolates between $v \neq 0$ and $v = 0$.

4. **Critical slowing down**: Near $T_c$, fluctuations grow large and correlation times diverge. Moral judgments become increasingly inconsistent and take longer to stabilize—a signature of critical behavior.

**Historical Examples (Speculative Mapping)**

| Event | Interpretation |
|-------|----------------|
| French Revolution (1789-1794) | Rapid heating past $T_c$; dissolution of ancien régime moral boundaries |
| Post-WWII reconstruction | Slow cooling; reconstitution of international moral order |
| 2020 pandemic onset | Local heating; temporary boundary softening ("emergency ethics") |

**Experimental Signature**

**Protocol 5: Moral Temperature Measurement**

1. **Operationalize $T_{\text{moral}}$**: Survey measures of perceived social instability, normative disagreement, institutional trust.

2. **Measure boundary rigidity**: Use Protocol 1 across contexts with varying $T_{\text{moral}}$.

3. **Prediction**: Plot mixing angle $\theta$ against $T_{\text{moral}}$. Near $T_c$, $\theta$ should increase sharply (boundary becomes transparent).

4. **Critical scaling**: Near the transition, expect:
$$\theta(T) \propto |T - T_c|^{-\nu}$$
where $\nu$ is a critical exponent.

---

**Remark (The "Moral Vacuum" Problem)**: Just as the electroweak vacuum has nonzero Higgs VEV, the "moral vacuum"—the default state of social normativity—has nonzero boundary VEV. This is what makes moral boundaries *meaningful*. A universe with $v = 0$ everywhere would have no stable moral distinctions; bond types would mix freely at every threshold. The fact that we observe relatively stable moral categories suggests we live in the **broken phase** at ordinary moral temperatures.
...
### 8.5 Protocol 5: Moral Phase Transition

**Setup**: Measure boundary rigidity across contexts with varying "moral temperature" (operationalized via surveys of perceived social instability, normative disagreement, institutional trust).

**Measurement**: Mixing angle $\theta$ from Protocol 1, across high-$T$ and low-$T$ contexts.

**Prediction**: 
- Below critical temperature: $\theta$ small (rigid boundaries)
- Near critical temperature: $\theta$ increases sharply
- Above critical temperature: $\theta \approx \pi/2$ (maximal mixing, "anything goes")

**Critical scaling**: Near $T_c$, expect $\theta \propto |T - T_c|^{-\nu}$ with measurable critical exponent.

**Falsifier**: No relationship between social volatility and boundary permeability.
...
3. **Moral phase transitions**: Temperature-dependent symmetry restoration at boundaries
...
8. **Five experimental protocols**: Including critical behavior near moral phase transitions

The phase transition prediction is particularly striking: it suggests that moral boundaries are not fixed features of normative reality but **emergent structures** that can melt under sufficient social stress and reconstitute as conditions stabilize. This provides a principled account of why "emergency ethics" differs qualitatively from ordinary moral reasoning, and why post-crisis societies often struggle to re-establish clear normative boundaries.
