...
class ScentAnalyzer:
    ...
    NIH_PATTERNS = {
        "python": {
            "custom_string": re.compile(
                r"def\s+(my_|custom_)?(split|join|strip|replace|lower|upper)\s*\("
            ),
        ...
        "c": {
            "custom_string": re.compile(
                r"\b(my_|custom_)(strlen|strcpy|strcmp|strcat|strdup|memcpy|memset)\s*\("
            ),
            "custom_container": re.compile(
                r"struct\s+(my_|custom_)?(list|array|queue|stack|hashmap|hashtable)\b"
            ),
            "printf_logging": re.compile(
                r"printf\s*\([^)]*%(s|d)[^)]*\b(error|warn|debug)\b", re.IGNORECASE
            ),
        },
    }
    ...
    SMELL_PATTERNS = {
        "magic_number": re.compile(
            r'(?<!["\'\w])(?<!\.)\b(?!0\b|1\b|2\b|-1\b)([-+]?\d{2,}(?:\.\d+)?)\b(?!["\'])'
        ),
        "deep_nesting": re.compile(
            r"^(\s{16,}|\t{4,})\S", re.MULTILINE
    ...
    def _calculate_file_score(self, metrics: FileSmellMetrics):
        ...
        nih = metrics.nih
        nih_count = (
            nih.custom_string_functions
            + nih.custom_container_classes
            + nih.custom_http_clients
            + nih.custom_json_parsers
    ...
    def _calculate_scores(self, report: ScentReport):
        ...
        # NIH score (0-100)
        nih_total = sum(
            fm.nih.custom_string_functions
            + fm.nih.custom_container_classes
            + fm.nih.custom_http_clients
            + fm.nih.trivial_wrappers
        ...
        # Smell score
        smell_total = sum(
            fm.smells.long_functions
            + fm.smells.very_long_functions * 2
            + fm.smells.deep_nesting
            + fm.smells.long_parameter_lists
            for fm in self.file_metrics
        )
        ...
        for fm in self.file_metrics:
            ...
            if fm.smells.deep_nesting:
                issue_counts["Deep nesting"] = (
                    issue_counts.get("Deep nesting", 0) + fm.smells.deep_nesting
                )
            if fm.smells.magic_numbers > 5:
                issue_counts["Magic numbers"] = issue_counts.get("Magic numbers", 0) + 1
            if fm.outdated.deprecated_apis:
                issue_counts["Deprecated APIs"] = (
                    issue_counts.get("Deprecated APIs", 0) + fm.outdated.deprecated_apis
                )
            if fm.nih.custom_logging:
                issue_counts["Print-based logging"] = (
                    issue_counts.get("Print-based logging", 0) + fm.nih.custom_logging
                )
...
def main():
    ...
    # Freshness emoji
    emoji = {"FRESH": "🌿", "STALE": "🍂", "MOLDY": "🍄", "ROTTEN": "🦨"}.get(
