...
Reframed prompt:"""
        ...
        return self._call_llm(
            system=self.TRANSFORMER_SYSTEM,
            user=user_msg,
            max_tokens=500
        )
...
Please rate this response."""
    ...
    def run_trial(self, prompt: str, framing_key: str, num_judges: int = 3) -> Trial:
        ...
        # Calculate averages
        raw_overall = [s.get("overall", 5) for s in trial.raw_scores.values() if isinstance(s.get("overall"), (int, float))]
        framed_overall = [s.get("overall", 5) for s in trial.framed_scores.values() if isinstance(s.get("overall"), (int, float))]
        
        trial.raw_avg_score = sum(raw_overall) / len(raw_overall) if raw_overall else 5.0
        trial.framed_avg_score = sum(framed_overall) / len(framed_overall) if framed_overall else 5.0
    ...
    def run_experiment(self, 
                       ...
                       num_judges: int = 3) -> ExperimentResults:
        ...
        for framing_key in framing_keys:
            ...
            framing_result.improvement_pct = (
                ...
                / framing_result.avg_raw_score * 100
            )
    ...
    def generate_report(self, results: ExperimentResults) -> str:
        ...
        lines.append("> Some framings (fun, expert, pirate, etc.) may produce better results than neutral prompts.")
        lines.append("")
        ...
        for i, (key, improvement) in enumerate(results.rankings):
            ...
            lines.append(f"| {emoji} {i+1} | {fr['framing_name']} | {improvement:+.1f}% | {fr['framed_wins']} | {fr['raw_wins']} |")
...
Available built-in framings:
  ...
  skeptical, storyteller, confused
    ...
    parser.add_argument("--trials", type=int, default=5, help="Number of prompts to test (default: 5)")
    ...
    parser.add_argument("--framing", type=str, default=None, help="Single framing to test (e.g., 'pirate')")
    parser.add_argument("--framings", type=str, default=None, help="Comma-separated framings (e.g., 'fun,pirate,expert')")
    parser.add_argument("--framings-csv", type=str, default=None, help="CSV file with custom framings")
    parser.add_argument("--prompts-file", type=str, default=None, help="File with prompts (one per line or CSV)")
    parser.add_argument("--custom-instruction", type=str, default=None, 
                        help="Custom framing instruction (use with --framing custom)")
    parser.add_argument("--list-framings", action="store_true", help="List available framings and exit")
    ...
    if args.dry_run:
        ...
        print(f"  Per framing: {calls_per_framing} = {args.trials} transforms + {args.trials*2} responses + {args.trials*args.judges*2} judgments")
        return
