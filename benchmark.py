...
def parse_result(data: dict) -> ActualResult:
    ...
    try:
        ...
        return ActualResult(
            ...
            resilience_rating=scores.get(
                "shield_rating", p.get("shield_rating", "UNKNOWN")
            ),
            resilience_score=scores.get(
                "resilience_score", p.get("resilience_score", 0)
            ),
            complexity_risk=scores.get(
                "complexity_risk", p.get("complexity_risk", "UNKNOWN")
            ),
...
def check_prediction(pred: Prediction, actual: ActualResult) -> dict:
    ...
    quadrant_close = (
        (
            pred.expected_quadrant == "BUNKER"
            and actual.quadrant in ["BUNKER", "FORTRESS"]
        )
        or (
            pred.expected_quadrant == "FORTRESS"
            and actual.quadrant in ["FORTRESS", "BUNKER"]
        )
        or (
            pred.expected_quadrant == "GLASS HOUSE"
            and actual.quadrant in ["GLASS HOUSE", "BUNKER"]
        )
        or (
            pred.expected_quadrant == "BUNKER"
            and actual.quadrant in ["BUNKER", "GLASS HOUSE"]
        )
    )
    ...
    freshness_close = (
        ...
        or (
            pred.expected_freshness == "STALE"
            and actual.freshness in ["STALE", "FRESH", "MOLDY"]
        )
        or (
            pred.expected_freshness == "MOLDY"
            and actual.freshness in ["MOLDY", "STALE", "ROTTEN"]
        )
    )
    ...
    avg_confidence = (
        pred.quadrant_confidence
        + pred.resilience_confidence
        + pred.freshness_confidence
    ) / 3
...
def generate_report(results: list[tuple[Prediction, ActualResult, dict]]) -> str:
    ...
    for pred, actual, check in results:
        ...
        q_emoji = (
            "✅"
            if check["quadrant_match"]
            else "🟡" if check["quadrant_close"] else "❌"
        )
        ...
        if too_small:
            ...
            r_emoji = (
                "✅"
                if check["resilience_match"]
                else "🟡" if check["resilience_close"] else "❌"
            )
        ...
        f_emoji = (
            "✅"
            if check["freshness_match"]
            else "🟡" if check.get("freshness_close") else "❌"
        )
    ...
    lines.append("")
    lines.append(
        f"**Overall Prediction Accuracy: {avg_score:.1f}%** (Raw: {avg_raw:.1f}%)"
    )
    lines.append("")
    ...
    high_conf = [
        ...
        if a.success and c.get("avg_confidence", 50) >= 70
    ]
    low_conf = [
        (p, a, c)
        for p, a, c in results
        if a.success and c.get("avg_confidence", 50) < 50
    ]

    if high_conf:
        high_conf_accuracy = sum(c.get("raw_score", 0) for _, _, c in high_conf) / len(
            high_conf
        )
        lines.append(
    ...
    if low_conf:
        low_conf_accuracy = sum(c.get("raw_score", 0) for _, _, c in low_conf) / len(
            low_conf
        )
        lines.append(
...
def main():
    ...
    parser.add_argument(
        "--only", type=str, help="Test only specific repo (e.g., 'antirez/sds')"
    )
    ...
    for pred in predictions:
        data = run_prometheus(
            pred.repo, pred.library_mode, include_security=has_security
        )
    ...
    close_count = sum(
        ...
        if a.success and c["quadrant_close"] and not c["quadrant_match"]
    )
