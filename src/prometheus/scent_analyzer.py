...
class PatternDetector:
    ...
    PATTERNS = {
        ...
        "circuit_states": re.compile(
            r"\b(HALF_OPEN|CircuitState|circuit_state|circuit_breaker|CircuitBreaker)\b"
        ),
        "circuit_state_machine": re.compile(
            r"\b(OPEN|CLOSED)\b.*\b(OPEN|CLOSED|HALF_OPEN)\b", re.DOTALL
        ),
        "failure_threshold": re.compile(
            r"\b(failure_threshold|failureThreshold|failure_rate|error_threshold|failure_count)\s*[=:]\s*\d+"
        ),
        "fallback": re.compile(
            r"\b(@Fallback|with_fallback|fallbackMethod|on_fallback|fallback_handler)\b"
        ),
        # Retry patterns
        "retry_decorator": re.compile(r"@(retry|Retry|Retryable|with_retry|retrying)"),
        "exponential_backoff": re.compile(
            r"\b(exponential|ExponentialBackoff|expo_backoff|backoff\.expo)\b"
        ),
        "max_retries": re.compile(
            r"\b(max_retries|maxRetries|max_attempts|maxAttempts|retry_count)\s*[=:]\s*\d+"
        ),
        "jitter": re.compile(r"\b(jitter|add_jitter|randomize|random_delay)\b"),
        "retry_loop": re.compile(
            r"for\s+\w+\s+in\s+range\s*\([^)]*retry|while.*retry|attempts?\s*[<>=]+\s*max"
        ),
        # Timeout patterns
        "timeout_config": re.compile(
            r"\b(timeout|Timeout|TIMEOUT|connect_timeout|read_timeout|socket_timeout)\s*[=:]\s*\d+"
        ),
        "timeout_exception": re.compile(
            r"\b(TimeoutError|TimeoutException|SocketTimeoutException|ConnectTimeoutException)\b"
        ),
        "async_timeout": re.compile(
            r"\b(asyncio\.timeout|async_timeout|with_timeout|wait_for.*timeout)\b"
        ),
        # Bulkhead patterns
        "thread_pool": re.compile(
            r"\b(ThreadPoolExecutor|thread_pool|ThreadPool|Executors\.newFixedThreadPool)\b"
        ),
        "semaphore": re.compile(r"\b(Semaphore|BoundedSemaphore|asyncio\.Semaphore)\b"),
        "connection_pool": re.compile(
            r"\b(pool_size|max_connections|maxPoolSize|connectionPoolSize)\s*[=:]\s*\d+"
        ),
        "queue_limit": re.compile(
            r"\b(max_queue|queue_size|maxQueueSize|bounded_queue)\s*[=:]\s*\d+"
        ),
        ...
        "trace_span": re.compile(
            r"\b(span|Span|tracer\.start|with_span|@trace|createSpan|startSpan)\b"
        ),
        ...
        "dependency_check": re.compile(
            r"\b(check_dependency|checkDependency|dependency_health|ping_database|ping_redis)\b"
        ),
...
class Aegis:
    ...
    def _map_lang_metrics_to_file_metrics(
        self, lang: LanguageResilienceMetrics, file: FileResilienceMetrics, content: str
    ):
    ...
    def _calculate_file_score(self, m: FileResilienceMetrics) -> float:
        ...
        if self.library_mode:
            ...
            has_boundaries = (
                ...
                or has_db > 0
                or has_file > 0
                or m.timeouts.missing_timeouts > 0  # detected HTTP without timeout
                or m.retries.library_imports  # has retry lib = probably needs it
                or m.circuit_breakers.library_imports  # has circuit breaker = needs it
                or m.error_handling.try_blocks > 0  # has try/catch = probably needs it
            )
    ...
    def _calculate_scores(self, report: AegisReport):
        ...
        report.circuit_breaker_score = min(
            100,
            (
                cb_files / n * 50
        ...
        if self.library_mode:
            ...
            report.timeout_score = min(
                100, max(0, (timeout_count / max(1, n) * 20) - (missing_count * 5))
            )

        # Bulkhead score
        bulkhead_patterns = sum(
            fm.bulkheads.thread_pool_configs
            + fm.bulkheads.semaphore_usage
        ...
        degradation_patterns = sum(
            fm.degradation.feature_flags
            + fm.degradation.default_returns
            + fm.degradation.cache_fallbacks
            + fm.degradation.fallback_values
            for fm in self.file_metrics
        )
        ...
        health_patterns = sum(
            fm.health_checks.health_endpoints
            + fm.health_checks.liveness_probes
            + fm.health_checks.readiness_probes
            + fm.health_checks.dependency_checks
            for fm in self.file_metrics
        )
        if self.library_mode:
            report.health_check_score = min(
                100, health_patterns * 20 + 50
            )  # Base score for libraries
        else:
            ...
            recs.append(
                {
                    ...
                    "libraries": [
                        "FastAPI health (Python)",
                        "Terminus (JS)",
                        "Spring Actuator (Java)",
                    ],
                }
            )
